...
namespace aw::core {
    ...
    template<typename CharType, typename Traits = CharTraits<CharType>, typename Allocator = DefaultAllocator<
        CharType> >
    ...
    public:
        ...
        template<usize N>
        StringTemplate(const CharType (&str)[N], Allocator allocator = Allocator())
            : StringTemplate(str, allocator) {
        }
        ...
        inline usize get_length() const { return m_Size ? m_Size - 1 : 0; }
        ...
        inline CharType *begin() { return m_Data; }
        inline const CharType *begin() const { return m_Data; }
        inline CharType *end() { return m_Data + m_Size; }
        inline const CharType *end() const { return m_Data + m_Size; }
    ...
    public:
        StringViewTemplate() = default;

        StringViewTemplate(const CharType *str, usize size)
            : m_Data(str),
              m_Size(size) {
        }

        StringViewTemplate(const CharType *str)
            : m_Data(str),
              m_Size(CharTraits<CharType>::length(str, CharType(0))) {
        }

        template<typename Traits, typename Allocator>
        StringViewTemplate(const StringTemplate<CharType, Traits, Allocator> &str)
            : m_Data(*str),
              m_Size(str.get_allocated_size())
        {}

        StringViewTemplate(const StringViewTemplate &other) = default;
        StringViewTemplate(StringViewTemplate &&other) noexcept = default;
        StringViewTemplate &operator=(const StringViewTemplate &other) = default;
        StringViewTemplate &operator=(StringViewTemplate &&other) noexcept = default;
